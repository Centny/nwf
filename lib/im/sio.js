...
SIO.prototype.onconn = function(c) {
	...
	c.on('li', function(args) {
		console.log("login from %s by", c.conn.remoteAddress, args);
		self.IM.uli_(args, {
			id: c.ccid,
			cback: function(v) {
				if (v.code === 0) {
	...
	function clo(args) {
		console.log("user login by %s", args);
		if (!c.R || !self.dm[c.R]) {
			return;
		}
	...
	c.on('ur', function() {
		console.log("list unread from %s", c.conn.remoteAddress);
		if (!c.R) {
			c.emit("err", {
				"err": "not login",
			});
